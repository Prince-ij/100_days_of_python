...
Original file is located at
    https://colab.research.google.com/drive/1l0Wny8_kFddPRssbxo27un0X2SLa0lRL
...
df_apps_clean[df_apps_clean.App == 'Instagram']['App'].count()
...
top_50 = df_apps_clean.sort_values('Reviews', ascending=False)[:50]
top_50.loc[top_50['Type'] == 'Paid'].count()
...
px.pie(labels=ratings.index, values=ratings.values, title='Content Rating', names=ratings.index)
...
df_apps_clean.Installs = pd.to_numeric(df_apps_clean.Installs.str.replace('[+,]', '', regex=True))

df_apps_clean.loc[df_apps_clean['Installs'] > 10000000].count()['Installs']

df_apps_clean.loc[df_apps_clean['Installs'] == 1]
...
bar = px.bar(x=top_10_category.index, y=top_10_category.values, title='TOP 10 App Categories')
bar.show()

"""### Vertical Bar Chart - Highest Competition (Number of Apps)"""

category_installs = df_apps_clean.groupby('Category').agg({'Installs': 'sum'}).sort_values('Installs', ascending=False)
...
category_installs['Apps'] = df_apps_clean.groupby('Category').agg({'App': 'count'})
category_installs.sort_values('Installs', ascending=False, inplace=True)
category_installs[['Apps', 'Installs']].head()
...
scatter = px.scatter(
    ...
    y='Installs',
    size='Apps',
    hover_name=category_installs.index,
    color='Apps',
...
bar.update_layout(xaxis_title='Genre', yaxis_title='Numner of Apps', coloraxis_showscale=False)
bar.show()
